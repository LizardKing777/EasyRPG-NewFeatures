...
int Game_Map::GetTileID1(int x, int y, int layer) {


	int tile_index = x + y * GetTilesX();
	int tile_raw_id1 = map->upper_layer[tile_index];
	int tile_id1 = 0;

	if (tile_raw_id1 >= BLOCK_E) {
		tile_id1 = tile_raw_id1 - BLOCK_E;
		tile_id1 = map_info.upper_tiles[tile_id1] + BLOCK_E_INDEX;

	}
	else if (tile_raw_id1 >= BLOCK_D) {
		/*tile_id = (tile_raw_id - BLOCK_D) / BLOCK_D_STRIDE + BLOCK_D_INDEX;
		int autotile_id = (tile_raw_id - BLOCK_D) % BLOCK_D_STRIDE;
		Output::Debug(" {} {} {}", tile_id, autotile_id, tile_raw_id);*/
		//return tile_id;
	...
	else if (tile_raw_id1 >= BLOCK_C) {
		tile_id1 = (tile_raw_id1 - BLOCK_C) / BLOCK_C_STRIDE + BLOCK_C_INDEX;

	}
	else if (map->upper_layer[tile_index] < BLOCK_C) {
		tile_id1 = tile_raw_id1 / BLOCK_B_STRIDE;
	}
